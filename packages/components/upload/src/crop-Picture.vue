<template>
  <div :class="bem.b()">
    ...
    <div :class="bem.e('operate')">
      <!-- 图片伸缩 -->
      <div :class="bem.e('operate-left')">
        <tl-icon icon="fa-minus" @click.stop="zoomOut" />
        <tl-slider v-model="scalePercent" :class="bem.e('operate-scale-bar')" />
        <tl-icon icon="fa-plus" @click.stop="zoomIn" />
      </div>
      <!-- 图片旋转 -->
      <div :class="bem.e('operate-right')">
        <tl-icon icon="fa-minus" @click.stop="rotateCounterClockwise" />
        <tl-slider v-model="rotationPercent" :class="bem.e('operate-scale-bar')" />
        <tl-icon icon="fa-plus" @click.stop="rotateClockwise" />
      </div>
    </div>
  </div>
</template>
...
const bem = createNamespace("image-crop");
...
const initCropArea = () => {
  ...
  context.fillRect(0, rectY + rectSize, canvas.width, canvas.height - (rectY + rectSize)); // 下方遮罩
  context.fillRect(0, rectY, rectX, rectSize); // 左侧遮罩
  context.fillRect(rectX + rectSize, rectY, canvas.width - (rectX + rectSize), rectSize); // 右侧遮罩
};
...
const handleCrop = async () => {
  ...
  // 获取2D上下文对象
  const tempContext: any = newCanvas.getContext("2d") as CanvasRenderingContext2D;
